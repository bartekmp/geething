...
els.notificationSoundEnabled.addEventListener('change', async () => {
  await saveSettings({ notificationSoundEnabled: els.notificationSoundEnabled.checked });
  refreshSoundUi();
});
els.notificationSoundType.addEventListener('change', async () => {
  await saveSettings({ notificationSoundType: els.notificationSoundType.value });
  refreshSoundUi();
});
...
els.notificationSoundVolume.addEventListener('change', () =>
  saveSettings({ notificationSoundVolume: Number(els.notificationSoundVolume.value) / 100 }),
);
...
els.customSoundRemove.addEventListener('click', async () => {
  try {
    ...
    await saveSettings({ customSoundName: '', customSoundDuration: 0 });
    refreshSoundUi();
  } catch (err) {
    showSoundError(err.message || String(err));
  }
